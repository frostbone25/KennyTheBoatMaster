...
KTBM_TablePrint = function(tbl, indent)
    if not indent then 
        indent = 0;
    end
    ...
    for k, v in pairs(tbl) do
        ...
        elseif (type(v) == "table") then
            toprint = toprint .. KTBM_TablePrint(v, indent + 2) .. ",\r\n";
        else
...
KTBM_VectorToString = function(vectorValue)
    local stringValue = "";
...
KTBM_ColorToString = function(colorValue)
    local stringValue = "";
...
KTBM_ColorToRGBColorToString = function(colorValue)
    local stringValue = "";
...
KTBM_Enum_TonemapType_ToString = function(valueType)
    local stringValue = "";
...
KTBM_Enum_T3LightEnvType_ToString = function(valueType)
    local stringValue = "";
...
KTBM_Enum_T3LightEnvShadowType_ToString = function(valueType)
    local stringValue = "";
...
KTBM_Enum_T3LightEnvShadowQuality_ToString = function(valueType)
    local stringValue = "";
...
KTBM_Enum_T3LightEnvLODBehavior_ToString = function(valueType)
    local stringValue = "";
...
KTBM_Enum_HBAOParticipationType_ToString = function(valueType)
    local stringValue = "";
...
KTBM_Enum_T3LightEnvEnlightenBakeBehavior_ToString = function(valueType)
    local stringValue = "";
...
KTBM_Enum_T3LightEnvMobility_ToString = function(valueType)
    local stringValue = "";
...
KTBM_Enum_T3LightEnvGroup_ToString = function(valueType)
    local stringValue = "";
...
KTBM_Number_ToString = function(propertyName, propertyValue)
    local stringValue = "";

    if (propertyName == "EnvLight - Type") then
        stringValue = stringValue .. KTBM_Enum_T3LightEnvType_ToString(propertyValue);
    elseif (propertyName == "EnvLight - HBAO Participation Type") then
        stringValue = stringValue .. KTBM_Enum_HBAOParticipationType_ToString(propertyValue);
    elseif (propertyName == "EnvLight - Mobility") then
        stringValue = stringValue .. KTBM_Enum_T3LightEnvMobility_ToString(propertyValue);
    elseif (propertyName == "EnvLight - Shadow Type") then
        stringValue = stringValue .. KTBM_Enum_T3LightEnvShadowType_ToString(propertyValue);
    elseif (propertyName == "EnvLight - Shadow Quality") then
        stringValue = stringValue .. KTBM_Enum_T3LightEnvShadowQuality_ToString(propertyValue);
    else
...
KTBM_PropertiesListToStringList = function(agentName, givenPropertyNamesList)
    ...
    for i, givenPropertyName in ipairs(givenPropertyNamesList) do
        ...
        local sceneAgentPropertyValue = KTBM_AgentGetProperty(agentName, givenPropertyName, KTBM_RelightDev_SceneObject);
        local sceneAgentPropertyValueType = tostring(TypeName(sceneAgentPropertyValue));

        --print out the value
        if (sceneAgentPropertyValueType == "number") then
            stringValue = stringValue .. KTBM_Number_ToString(givenPropertyName, sceneAgentPropertyValue);
        elseif (sceneAgentPropertyValueType == "table") then
            local tableType = KTBM_GetTableType(sceneAgentPropertyValue);
        
            if (tableType == "color") then
                 stringValue = stringValue .. KTBM_ColorToRGBColorToString(sceneAgentPropertyValue);
            else
...
KTBM_TimeSecondsFormatted = function(number_timeSecondsValue)
