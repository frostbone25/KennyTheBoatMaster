...
KTBM_AgentHasProperty = function(agentName, propertyString, kScene)
...
KTBM_PropertySet = function(agent, propertyString, propertyValue)
...
KTBM_AgentSetProperty = function(agentName, propertyString, propertyValue, kScene)
    local agent = AgentFindInScene(agentName, kScene)
    KTBM_PropertySet(agent, propertyString, propertyValue)
end

--forcibly sets a property on an agent by name
KTBM_AgentForceSetProperty = function(agentName, propertyString, propertyValueType, propertyValue, kScene)
    local agent = AgentFindInScene(agentName, kScene)
...
KTBM_SetPropertyOnAgentsWithPrefix = function(kScene, prefixString, propertyString, propertyValue)
    ...
    for i, agent_object in pairs(scene_agents) do
        ...
        if (string.match)(agent_name, prefixString) then
            KTBM_AgentSetProperty(agent_name, propertyString, propertyValue, kScene)
        end
    end
end

--sets a property on all cameras in a scene
KTBM_SetPropertyOnAllCameras = function(kScene, propertyString, propertyValue)
    --get all agents in the scene
    local scene_agents = SceneGetAgents(kScene)
...
KTBM_SetTexturesOnAgentWithNamePrefix = function(agentName, texturePrefixString, newPropertyValue, cacheObjectListName, kScene)
    local agent = AgentFindInScene(agentName, kScene);
...
KTBM_SetTexturesOnAgentsWithNamePrefixes = function(agentNamePrefix, texturePrefixString, newPropertyValue, cacheObjectListName, kScene)
    local scene_agents = SceneGetAgents(kScene)
    ...
    for i, agent_object in pairs(scene_agents) do
        ...
        if (string.match)(agent_name, agentNamePrefix) then
            KTBM_SetTexturesOnAgentWithNamePrefix(agent_name, texturePrefixString, newPropertyValue, cacheObjectListName, kScene)
        end
    end
end

KTBM_SetDiffuseTexturesOnAgentsWithNamePrefixes = function(agentNamePrefix, newPropertyValue, cacheObjectListName, kScene)
    local scene_agents = SceneGetAgents(kScene)
...
KTBM_PropertyGet = function(agent, propertyString)
    local agent_props = AgentGetRuntimeProperties(agent)
    return PropertyGet(agent_props, propertyString)
end

--gets a property on an agent by name
KTBM_AgentGetProperty = function(agentName, propertyString, kScene)
    local agent = AgentFindInScene(agentName, kScene)
    return KTBM_PropertyGet(agent, propertyString)
end

--gets properties on an agent by name
KTBM_AgentGetProperties = function(agentName, kScene)
    local agent = AgentFindInScene(agentName, kScene)
    return AgentGetProperties(agent)
end

--gets runtime properties on an agent by name
KTBM_AgentGetRuntimeProperties = function(agentName, kScene)
    local agent = AgentFindInScene(agentName, kScene)
