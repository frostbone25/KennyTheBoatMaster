...
- KTBM_Core_AgentExtensions_Properties.lua
- KTBM_Core_AgentExtensions_Transform.lua
- KTBM_Core_AgentExtensions_Utillity.lua
...
KTBM_DOF_AUTOFOCUS_UseCameraDOF = false;
KTBM_DOF_AUTOFOCUS_UseLegacyDOF = true;
...
KTBM_DepthOfFieldAutofocus_SetupDOF = function()
    ...
    if(KTBM_DOF_AUTOFOCUS_UseLegacyDOF == true) then
...
KTBM_DepthOfFieldAutofocus_PerformAutofocus = function()
    ...
    local currentCamera_agent = SceneGetCamera(KTBM_DOF_AUTOFOCUS_SceneObject); --Agent type
    ...
    local currentCamera_far_plane = KTBM_AgentGetProperty(currentCamera_name, "Clip Plane - Far", KTBM_DOF_AUTOFOCUS_SceneObject); --Number type
    local currentCamera_near_plane = KTBM_AgentGetProperty(currentCamera_name, "Clip Plane - Near", KTBM_DOF_AUTOFOCUS_SceneObject); --Number type
    ...
    local rejectionAngle = KTBM_DOF_AUTOFOCUS_Settings["TargetValidation_RejectionAngle"]; --goes from -1 to 1 (less than 0 is within the 180 forward facing fov of the given object)
    local rejectionDistance = KTBM_DOF_AUTOFOCUS_Settings["TargetValidation_RejectionDistance"]; --the max distance before the agent is too far from camera to do autofocus
    
    --list of valid target agents
    local validAgentsArray = {}; --Agent array
    
    --find all of the valid target objects in the scene
    for i, objectName in ipairs(KTBM_DOF_AUTOFOCUS_ObjectEntries) do
        ...
        object_agent = AgentFindInScene(objectName, KTBM_DOF_AUTOFOCUS_SceneObject); --Agent type
        
        --if the object exists
        if (object_agent) then
            --assign our variables
            object_isOnScreen = AgentIsOnScreen(object_agent);
            object_isVisible = KTBM_AgentGetProperty(objectName, "Runtime: Visible", KTBM_DOF_AUTOFOCUS_SceneObject);
            ...
            if (isValidTarget == true) and (KTBM_DOF_AUTOFOCUS_Settings["TargetValidation_IsOnScreen"]) and (object_isOnScreen == false) then isValidTarget = false; end
            
            --check if the target's visibillity is disabled
            if (isValidTarget == true) and (KTBM_DOF_AUTOFOCUS_Settings["TargetValidation_IsVisible"]) and (object_isVisible == false) then isValidTarget = false; end
            
            --check if the target is too far from camera
            if (isValidTarget == true) and (KTBM_DOF_AUTOFOCUS_Settings["TargetValidation_IsWithinDistance"]) and (object_distance > rejectionDistance) then isValidTarget = false; end

            --check if the target is not within the rejection angle relative to the camera
            if (isValidTarget == true) and (KTBM_DOF_AUTOFOCUS_Settings["TargetValidation_IsFacingCamera"]) and (object_vector_dot > rejectionAngle) then isValidTarget = false; end
            
            --perform a raycast, if the ray intersects with the scene then the target is occluded and therefore, not valid.
            if (isValidTarget == true) and (KTBM_DOF_AUTOFOCUS_Settings["TargetValidation_IsOccluded"]) and (KTBM_RaycastFromAgentToAgent(object_agent, currentCamera_agent) == true) then isValidTarget = false; end
    ...
    for x, validAgent in ipairs(validAgentsArray) do
        ...
        if (nearFocusTarget1_agent) then
            ...
            nearFocusTarget1_agent = KTBM_GetNearestAgent(currentCamera_agent, nearFocusTarget1_agent, validAgent);
        else
        ...
        if (farFocusTarget2_agent) then
            ...
            farFocusTarget2_agent = KTBM_GetFarthestAgent(currentCamera_agent, farFocusTarget2_agent, validAgent);
        else
    ...
    for i, name in ipairs(KTBM_DOF_AUTOFOCUS_GameplayCameraNames) do
        --if the current camera is infact a gameplay camera
        if (currentCamera_name == KTBM_DOF_AUTOFOCUS_GameplayCameraNames[i]) then
    ...
    local focalRange = KTBM_DOF_AUTOFOCUS_FocalRange --USER ADJUSTABLE (the bigger in range, the more in focus)
    ...
    if (KTBM_DOF_AUTOFOCUS_UseCameraDOF == true) then
        --Use the DOF properties on the camera itself
        KTBM_AgentSetProperty(currentCamera_name, "Depth Of Field Enabled", current_enabledDOF, KTBM_DOF_AUTOFOCUS_SceneObject)
        KTBM_AgentSetProperty(currentCamera_name, "Use High Quality DOF", true, KTBM_DOF_AUTOFOCUS_SceneObject)
        KTBM_AgentSetProperty(currentCamera_name, "Depth Of Field Type", 1, KTBM_DOF_AUTOFOCUS_SceneObject)
        KTBM_AgentSetProperty(currentCamera_name, "Depth Of Field Blur Strength", 3, KTBM_DOF_AUTOFOCUS_SceneObject)
        KTBM_AgentSetProperty(currentCamera_name, "Depth Of Field Coverage Boost", 1, KTBM_DOF_AUTOFOCUS_SceneObject)

        if(KTBM_DOF_AUTOFOCUS_UseHighQualityDOF == true) then
            KTBM_AgentSetProperty(currentCamera_name, "Use Bokeh", true, KTBM_DOF_AUTOFOCUS_SceneObject)

            KTBM_AgentSetProperty(currentCamera_name, "Bokeh Brightness Delta Threshold", KTBM_DOF_AUTOFOCUS_BokehSettings.BokehBrightnessDeltaThreshold, KTBM_DOF_AUTOFOCUS_SceneObject)
            KTBM_AgentSetProperty(currentCamera_name, "Bokeh Brightness Threshold", KTBM_DOF_AUTOFOCUS_BokehSettings.BokehBrightnessThreshold, KTBM_DOF_AUTOFOCUS_SceneObject)
            KTBM_AgentSetProperty(currentCamera_name, "Bokeh Blur Threshold", KTBM_DOF_AUTOFOCUS_BokehSettings.BokehBlurThreshold, KTBM_DOF_AUTOFOCUS_SceneObject)
            KTBM_AgentSetProperty(currentCamera_name, "Bokeh Min Size", KTBM_DOF_AUTOFOCUS_BokehSettings.BokehMinSize, KTBM_DOF_AUTOFOCUS_SceneObject)
            KTBM_AgentSetProperty(currentCamera_name, "Bokeh Max Size", KTBM_DOF_AUTOFOCUS_BokehSettings.BokehMaxSize, KTBM_DOF_AUTOFOCUS_SceneObject)
            KTBM_AgentSetProperty(currentCamera_name, "Bokeh Falloff", KTBM_DOF_AUTOFOCUS_BokehSettings.BokehFalloff, KTBM_DOF_AUTOFOCUS_SceneObject)
            KTBM_AgentSetProperty(currentCamera_name, "Max Bokeh Buffer Amount", KTBM_DOF_AUTOFOCUS_BokehSettings.MaxBokehBufferAmount, KTBM_DOF_AUTOFOCUS_SceneObject)
            KTBM_AgentSetProperty(currentCamera_name, "Bokeh Pattern Texture", KTBM_DOF_AUTOFOCUS_BokehSettings.BokehPatternTexture, KTBM_DOF_AUTOFOCUS_SceneObject)
        else
            KTBM_AgentSetProperty(currentCamera_name, "Use Bokeh", false, KTBM_DOF_AUTOFOCUS_SceneObject)
        end

        KTBM_AgentSetProperty(currentCamera_name, "Depth Of Field - Far", current_farFocusDistance, KTBM_DOF_AUTOFOCUS_SceneObject)
        KTBM_AgentSetProperty(currentCamera_name, "Depth Of Field Fall Off - Far", current_farFalloff, KTBM_DOF_AUTOFOCUS_SceneObject)
        KTBM_AgentSetProperty(currentCamera_name, "Depth Of Field Max - Far", current_farMax, KTBM_DOF_AUTOFOCUS_SceneObject)
        
        KTBM_AgentSetProperty(currentCamera_name, "Depth Of Field - Near", current_nearFocusDistance, KTBM_DOF_AUTOFOCUS_SceneObject)
        KTBM_AgentSetProperty(currentCamera_name, "Depth Of Field Fall Off - Near", current_nearFallof, KTBM_DOF_AUTOFOCUS_SceneObject)
        KTBM_AgentSetProperty(currentCamera_name, "Depth Of Field Max - Near", current_nearMax, KTBM_DOF_AUTOFOCUS_SceneObject)
    else
        --Use the DOF properties on the scene post processing itself
        KTBM_AgentSetProperty(KTBM_DOF_AUTOFOCUS_SceneObjectAgentName, "FX DOF Enabled", current_enabledDOF, KTBM_DOF_AUTOFOCUS_SceneObject)
        
        KTBM_AgentSetProperty(KTBM_DOF_AUTOFOCUS_SceneObjectAgentName, "FX DOF Far", current_farFocusDistance, KTBM_DOF_AUTOFOCUS_SceneObject)
        KTBM_AgentSetProperty(KTBM_DOF_AUTOFOCUS_SceneObjectAgentName, "FX DOF Far Falloff", current_farFalloff, KTBM_DOF_AUTOFOCUS_SceneObject)
        KTBM_AgentSetProperty(KTBM_DOF_AUTOFOCUS_SceneObjectAgentName, "FX DOF Far Max", current_farMax, KTBM_DOF_AUTOFOCUS_SceneObject)
        
        KTBM_AgentSetProperty(KTBM_DOF_AUTOFOCUS_SceneObjectAgentName, "FX DOF Near", current_nearFocusDistance, KTBM_DOF_AUTOFOCUS_SceneObject)
        KTBM_AgentSetProperty(KTBM_DOF_AUTOFOCUS_SceneObjectAgentName, "FX DOF Near Falloff", current_nearFallof, KTBM_DOF_AUTOFOCUS_SceneObject)
        KTBM_AgentSetProperty(KTBM_DOF_AUTOFOCUS_SceneObjectAgentName, "FX DOF Near Max", current_nearMax, KTBM_DOF_AUTOFOCUS_SceneObject)
    end
